...
;Détermine le programme comme utilisation prioritaire du processeur
SetBatchLines, -1
...
;Génère les informations et options dans le Systray
Menu, Tray, Tip, Fait Marquant RÉSEAUX
...
;Vérification de la version de FFSEC
VersionToUse = 1.38
...
If VersionFirefox =
{
	MsgBox, 262160, Firefox non présent, Firefox n'est pas installé sur votre machine.
	Run, http://ffsec/
	ExitApp
}
If VersionFirefox < 1.38
{
	MsgBox, 262160, Version de Firefox non compatible, Votre version de Firefox (%VersionFirefox%) n'est pas compatible avec l'outil FM-Réseau.`n`nIl vous faut au minimum la version %VersionToUse%.
	Run, http://ffsec/
	ExitApp	
}

;Récupération des équipements pour l'affichage dans la liste déroulante
...
;Variable pour déterminer si un routeur est un X ou Y
routy = _Y
routx = _X
Clipboard = ;Vidage du presse papier
OnMessage(0x200, "Informations") ;Fonction permettant d'afficher un message au passage de la souris sur un contrôle
WinGetPos,,,, TaskBar, ahk_class Shell_TrayWnd ;Détermine la hauteur de la taskbar
ecranprogress := (A_ScreenHeight - TaskBar - 160)
;Si la barre des taches est positionné de manière vertical on défini une taille standard
If Taskbar = %A_ScreenHeight%
	ecranprogress := 760

HeureLancement := A_Hour ":" A_Min ;Variable permettant de connaitre l'heure à laquelle le programme a été lancé
...
Gui, Add, Text, xm+45 y10 w300 h25 vFM caf0000 Center, ÉQUIPEMENT IMPACTÉ
...
Gui, Add, CheckBox, x60 y+5 vISO, Le site est isolé du réseau
...
Gui, Add, Text,x25 y+10 w330 vRe BackgroundTrans, Région : /
...
Gui, Add, Button,x105 y+10 w200 vPing gPing Disabled, Ping de l'équipement
...
Gui, Add, CheckBox, x90 y+3 vSFR gSFR, Ticket SFR non généré
...
Gui, Show, Autosize , Fait Marquant RÉSEAUX

Ticket:
ClipWait ;Attente qu'un contenu arrive dans le presse-papier
GuiControlGet, SFR ;Récupération de l'état de la checkbox SFR
If SFR = 0 ;Si la case n'est pas coché on vérifie le contenu du presse papier
{
	If RegExMatch(ClipBoard, "^((C)\d{8})$") ;On vérifie que le numéro de ticket SFR correspond
	{
		GuiControl,,TicketSFR,%Clipboard% ;On le place dans le champ Ticket SFR
		Hotkey, LButton , Off ;On désactive le système de copie automatique
	}
	Else
		Goto, Ticket ;Si le REGEX ne correspond pas on retourne à l'action Ticket
}
Return


Informations()
{ ;Fonction permettant d'afficher un message au passage de la souris sur un contrôle
	If A_GuiControl = LogoSFR
		Message := "Cliquez ici pour contacter la hotline SFR"
	Else If A_GuiControl = Ping
		Message := "Cliquez ici pour effectuer un ping de l'équipement"
	Else If A_GuiControl = ChoixHeure
		Message := "Indiquez ici l'heure de l'indisponibilité de l'équipement" 		
	Else If A_GuiControl = TicketSFR
		Message := "Saisissez ici le ticket SFR associé au routeur"
    Else If A_GuiControl = SFR
		Message := "Cliquez ici pour ouvrir le FM dans le cas où aucun ticket n'a été généré dans la BAL SFR.`nN'oubliez pas de générer un ticket en contactant la hotline SFR après l'ouverture du FM."
	Else If A_GuiControl = Ackno
		Message := "Cette option permet de positionner un acknowledge automatique`nde l'équipement après la génération de la fiche Service Manager"	     
...
;Script permettant de faire une copie automatique du ticket SFR présent dans le mail.
~LButton::
	Loop
	{
...
AC:
	...
	Gui, submit, nohide ;Récupération des valeurs du GUI
	;Récupère dans le fichier INI les infos et les stockent dans des variables.
	...
	;Si l'équipement est un routeur nous activons les cases spécifiques (Ticket SFR, Site Isolé) autrement nous les désactivons.
	if infotype = Routeur
	{
	...
	Else	
	{
		;Si le type d'équipement n'existe pas on met un /
		If infotype = ERROR
			infotype = /
	...
	InfoService = /
	GuiControl,text,re,Région : %inforegion%
...
Remplissage:
	...
	If ChoixEquipement =
	{
		MsgBox,262160, Équipement manquant, Merci d'indiquer l'équipement impacté.
	...
	If InfoRegion = /
	{
		MsgBox,262160, Equipement non référencé , L'équipement saisi n'existe pas dans la base de données Centreon.
	...
	{
		MsgBox,262160,Format du jour incorrect, N'oubliez pas de rajouter un "0" si vous êtes dans la tranche du 1 au 9.`n`nExemple : 08 au lieu de 8
	...
	{
		MsgBox,262160,Format du mois incorrect, N'oubliez pas de rajouter un "0" si vous êtes dans la tranche de Janvier à Septembre.`n`nExemple : 03 au lieu de 3
		GuiControl, Focus, ChoixMois
		Return
	}
	If Not RegExMatch(ChoixAnnee, "^((202)[0-9])$")
	{
		MsgBox,262160,Format de l"année incorrect, Le format de l'année doit être composé de 4 chiffres`n`nExemple : 2014 au lieu de 14`n`nUniquement les dates de 2014 à 2019 peuvent être utilisées.
	...
	{
		MsgBox,262160,Format de l'heure incorrect, Le format de l'heure doit être de la sorte : XX:XX.`n`nExemple : 08:23
	...
	{
		MsgBox,262160, Ticket SFR manquant, Merci d'indiquer le numéro du ticket SFR.
	...
	If (EtatSFR = 1 and EtatTicketSFR = 1)
	{
		...
		{
			MsgBox,262160,Numéro du ticket SFR incorrect, Le numéro du ticket SFR n'est pas au bon format.`n`nExemple : C13560321
			GuiControl, Focus, TicketSFR
			Return
		}
	}
	...
	{
		MsgBox, 262196, Heure non changée, L'heure d'indisponiblité est la même qu'à l'ouverture du programme !`n`nVoulez-vous vraiment envoyer le fait marquant avec comme heure d'indisponiblité %ChoixHeure% ?
		IfMsgbox, No
		{
			GuiControl, Focus, ChoixHeure
			Return
...
;On détermine le type d'équipement et sa criticité pour ouvrir le FM en conséquence (Message spécifique, appel astreinte opérateur, etc...)
	If (Iso = "1" and InfoType = "Routeur") ;Site isolé  
	{
		Mode = Isolé
		VueJ = 1
		If infosite2 <> /
		...
		Routeur := RegExReplace(ChoixEquipement, "(-RET-)\d{7}", "") ;Récupération du nom court du routeur	
		...
		Progress, w800 y%EcranProgress% fs13 M2 CW800000 CTFFFFFF zh0,Appelez l'astreinte opérateur au : `n0 810 810 311`n`nLe site isolé est : %siteimpacte%,,Important, Verdana
	}
	...
	Else If InfoType = Serveur de fichiers
	{
		...
		VueJ = 1
		FormatTime, AstreinteOperateur,, HH:mm
		Progress, w800 y%EcranProgress% fs13 M2 CW800000 CTFFFFFF zh0, Appelez l'astreinte opérateur au : `n0 810 810 311`n`nLe serveur de fichiers impacté est %ChoixEquipement%,,Important, Verdana
	}
...
Else If InfoType = Contrôleur de domaine
			{
			Equipe = SN1_C1_EXPLOITATION
			Equipement = Le contrôleur de domaine
			Equipement2 = du contrôleur de domaine
			Categorie = Autres
			Domaine = 00-Appli / Infra
			SousDomaine = Process-Faits Marquants
			Impact = 3
...
Hotkey, LButton , Off ;Désactivation de la copie automatique du numéro de ticket SFR
;Affichage du message d'information en haut de l'écran	
...
;Si la case Service Manager est coché on passe directement à ERICA V2
If SM = 1
	Goto, LancementERICAV2

;Si Service Manager est lancé on active la fenêtre
...
Loop {
	...
	PixElsearch, FoundX, FoundY, 0, 0, A_ScreenWidth, A_ScreenHeight, 0xFF00FF, 0, Fast
		If ErrorLevel = 0
			{
			MessageInfo("DOUBLE CLIQUEZ à droite du `'Numéro d`'Incident`' pour préremplir la fiche", 650)
			Gui, 2:Color, A00075			
			Break
			}
	}
...
;Auto-remplissage de la fiche SM en fonction du type d'équipement et sa criticité
MessageInfo("Préremplissage en cours de la fiche Service Manager", 470)
Gui, 2:Color, 008409
If Mode = Routeur
{
Clipboard =
...
Centreon nous retourne une indisponibilité du routeur %Routeur%.
...
Centreon nous retourne une indisponibilité des 2 routeurs suivants :
- %RouteurX%
- %RouteurY%
	
Par conséquent, le site %siteimpacte% est isolé du réseau.
...
Cordialement
	...
	Send, {TAB}SN2_R1_RESEAU{TAB 5}SUPERVISION`,{SPACE 2}(U146876){TAB 5}%InfoService%{TAB 4}SFR %TicketSFR%{TAB}%ChoixEquipement%{TAB 6}00-Appli / Infra{TAB 2}%ChoixJour%/%ChoixMois%/%ChoixAnnee% %ChoixHeure%:00{TAB 2}Process-Faits Marquants{TAB 4}2{TAB 3}2{TAB 2}[FM] Le site %siteimpacte% est isolé du réseau{TAB}
	Send, ^v
	}
Else
	{
...
Centreon nous retourne une indisponibilité %Equipement2% %ChoixEquipement%.

Merci de prendre en charge cet incident.

Cordialement
	)		
	Send, {TAB}%Equipe%{TAB 5}SUPERVISION`,{SPACE 2}(U146876){TAB 5}%InfoService%{TAB 5}%ChoixEquipement%{TAB 6}%Domaine%{TAB 2}%ChoixJour%/%ChoixMois%/%ChoixAnnee% %ChoixHeure%:00{TAB 2}%SousDomaine%{TAB 4}%Impact%{TAB 3}%Urgence%{TAB 2}[FM] %Equipement% %ChoixEquipement% ne répond plus au ping{TAB}
	Send, ^v
		}
...
MessageInfo("Vérifiez le contenu de l'incident puis appuyez sur la touche `'Ctrl Droit`' pour le sauvegarder", 760)
Gui, 2:Color, A00075
KeyWait, RControl, D
...
If VueJ = 1
	{
		MessageInfo("Cochez la case `'Vue en journalière`' placez votre curseur dans 'Nouvelle mise a jour' et appuyez sur la touche `'Ctrl Droit`' pour sauvegarder", 1000)
		Gui, 2:Color, A00075
		EtapeSuivante = LancementEricaV2
		KeyWait, RControl, D
		Sleep, 300
		Send, Coche de la case 'Vue en journalière'
		Sleep, 500
		Send, ^+{F4}
		Sleep, 2000
	}		
If Ackno = 1
	...
	If ErrorLevel
		{
		MsgBox, 262160, Erreur, L'Acknowledge n'a pas pu être mis en place.`nMerci de le mettre en place manuellement.
		Goto, LancementERICAV2
		}
	If Mode = Isolé
		{
...
LancementERICAV2:
MessageInfo("Lancement et authentification à ERICA V2", 400)
Gui, 2:Color, 008409
Gosub, EricaV2
WinWait, ERICA V2
clickfmErica:
...
MessageInfo("CLIQUEZ sur l'équipement impacté", 340)
KeyWait, LButton, D
clickSelection:
CoordMode, Pixel, Screen:
...
;Saisi du texte en fonction du mode (Routeur, Site isolé ou Autre)
If Mode = Routeur
	SendInput, Le routeur %Routeur% ne répond plus au ping
Else If Mode = Isolé
	SendInput, Le site %siteimpacte% est isolé du réseau
Else
	SendInput, %Equipement% %ChoixEquipement% ne répond plus au ping
...
if ErrorLevel = 1
	Goto, clicketape
If Mode = Routeur
	SendInput, Le routeur %Routeur% ne répond plus au ping{TAB}Analyse en cours{TAB}Ticket SFR : %TicketSFR%{TAB}
Else If Mode = Isolé
	SendInput, Le site %siteimpacte% est isolé du réseau{TAB}Analyse en cours{TAB}Ticket SFR : %TicketSFR%{TAB}
Else
	SendInput, %Equipement% %ChoixEquipement% ne répond plus au ping{TAB 2}Analyse en cours{TAB}
Sleep, 500
Send, {ENTER}
Sleep, 1500
If Mode = Isolé
...
Send, {ENTER}
MessageInfo("Récupération automatique du 2ème routeur lié au fait marquant", 560)
Gui, 2:Color, 008409
Sleep, 1500
...
Gui, 3:Add, Text, w300 ,Numéro de téléphone : 0 805 770 112
Gui, 3:Font, s13 bold c0B610B, Segoe UI Semibold
Gui, 3:Add, Text, w300 ,Numéro de contrat : 109283
Gui, 3:Font, s13 bold c000000, Segoe UI Semibold
Gui, 3:Add, Text, w300 ,Société : RTE
...
If InStr(Contents, "perdus = 1") or InStr(Contents, "Impossible") and InStr(Contents, "perdus = 0")
	...
	MsgBox,262160,Information, %EquipementSelection% ne répond pas au ping !
	}
Else	
	{
	GuiControl,,Ping,PING OK
	Msgbox,262208, Information, %EquipementSelection% répond au ping !
	}
...
GuiControl,,Ping,Ping de l'équipement
Return

;Ferme la fenêtre de contact Hotline SFR
2GuiClose:
Gui, 2:Destroy
Return
...
;Permet de déplacer le GUI en maintenant le clic dessus
uiMove:
PostMessage, 0xA1, 2,,, A 
Return

;Fonction mettant à jour la barre d'information en haut de l'écran
...
;Fonctions nécessaires à l'auto-completion
; ======================================================================================================================
